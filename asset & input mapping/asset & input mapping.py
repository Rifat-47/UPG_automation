...
if login_json.status_code == 200:
    ...
    for inputy in current_child_inputs:
        asset_id = inputy["asset_id"]
        map_id = inputy['id']
        delete_input_request_json = requests.delete(f"https://upgapstg.brac.net/upg-participant-selection/api/v1/input/asset-input-mapping/by-asset-id/{asset_id}/by-id/{map_id}",
                                                    headers={'Authorization': f"Bearer {access_token}"})
    ...
    for inputz in parent_inputs:
        data = {
            "list": [{
                ...
                "category_wise_input_id": inputz["input_category_wise_input_id"]
            }]
        }
        """adding input map in child program"""
        child_input_update_request = requests.post('https://upgapstg.brac.net/upg-participant-selection/api/v1/input/asset-input-mapping',
