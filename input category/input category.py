...
if login_json.status_code == 200:
    ...
    print('Parent inputs category: ', len(parent_inputs))
    print('Current child inputs category: ', len(current_child_inputs))

    """delete child's existing inputs category"""
    print("Deleting existing input category, please wait...")
    child_deleted_input = 0
    for inputy in current_child_inputs:
        cohort_id = selected_child_cohort_info['cohort_id']
        input_id = inputy['id']
        delete_input_request_json = requests.delete(f"{base_url}/upg-participant-selection/api/v1/input/{input_id}",
                                                    headers={'Authorization': f"Bearer {access_token}"})
        delete_input_request = json.loads(delete_input_request_json.content)

        if 'result' in delete_input_request:
            if delete_input_request['result']['is_success']:
                child_deleted_input += 1
        else:
            print(f"Error: Can't delete input category, id {input_id}")

    if len(current_child_inputs) == child_deleted_input:
        print(f"All existing inputs category deleted. Total {child_deleted_input} deleted.")
    else:
        print(f'{child_deleted_input} out of {len(current_child_inputs)} inputs category has been deleted.')
        print(f"Delete all input category from db input_category table, check sql query folder(input category).")
        sys.exit(10)

    """adding input category setup data"""
    print('Updating input category setup, please wait...')
    ...
    for inputx in parent_inputs:
        ...
        child_input_update_request = requests.post(f'{base_url}/upg-participant-selection/api/v1/input',
