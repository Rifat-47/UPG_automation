"""mandatory: delete all swot and swot type from child if any remains, check sql query folder(swot)"""
""" query demo:
SELECT * from swot WHERE cohort_id='child cohort id' ALLOW FILTERING;
SELECT * from swot_type WHERE cohort_id='child cohort id' ALLOW FILTERING;"""
...
try:
    ...
    if login_json.status_code == 200:
        ...
        parent_all_swot_name_json = requests.get(
            f'{base_url}/upg-enrollment/api/v1/swot/type/cohort/{parent_cohort_id}',
            headers={'Authorization': f"Bearer {access_token}"})
        ...
        #     f'{base_url}/upg-enrollment/api/v1/swot/type/cohort/{child_cohort_id}',
        #     headers={'Authorization': f"Bearer {access_token}"})
        ...
        #                         f'{base_url}/upg-enrollment/api/v1/swot/type/update/{child_swot_name["id"]}/cohort/{child_cohort_id}',
        ...
        for parent_swot_name in parent_all_swot_data:
            if not parent_swot_name.get('checked'):
                ...
                child_add_swot_name_json = requests.post(
                    f'{base_url}//upg-enrollment/api/v1/swot/type',
                    json=data, headers={'Authorization': f"Bearer {access_token}"})
        ...
        child_current_swot_map_data_json = requests.get(
            f'{base_url}/upg-enrollment/api/v1/swot/cohort/{child_cohort_id}',
            headers={'Authorization': f"Bearer {access_token}"})
        ...
        parent_swot_map_data_json = requests.get(
            f"{base_url}/upg-enrollment/api/v1/swot/cohort/{parent_cohort_id}",
            headers={'Authorization': f"Bearer {access_token}"})
        ...
        #                     update_child_data_json = requests.patch(
        #                         f'{base_url}/upg-enrollment/api/v1/swot/update/{child_swot_map["id"]}/cohort/{child_swot_map["cohort_id"]}/swot-type/{child_swot_map['swot_type_id']}',
        #                         json=data, headers={'Authorization': f"Bearer {access_token}"})
        #                     if update_child_data_json.status_code == 200:
        ...
        #             url = (f'{base_url}/upg-enrollment/api/v1/swot/update/'
        ...
        for parent_swot_map in parent_swot_map_data_info['resultset']:
            if not parent_swot_map.get('counted') and parent_swot_map['swot_type_name'] in child_swot_name_dic:
                ...
                child_add_swot_map_request_json = requests.post(f'{base_url}/upg-enrollment/api/v1/swot/',json=data,
                    headers={'Authorization': f"Bearer {access_token}"})
                if child_add_swot_map_request_json.status_code == 200:
                    child_added_swot_map += 1
                else:
                    print(f"Error: Can't update swot map, {data}")
